...
- 首译每个 item 发送稳定的 `sentence_context` 邻接字幕快照，只供远端理解；必须只返回当前 item。快照基于完整 transcript，跨 batch 和递归缩批不变；并发 work unit 不共享 session 上下文
...
| `PROOFREAD_THINKING` / `PROOFREAD_REASONING_EFFORT` | 可选 provider 推理覆盖；留空不发送，使用 provider 默认 |
...
`PROOFREAD_BATCH_SIZE` 和 `PROOFREAD_CONCURRENCY` 分别控制单次请求大小与并发在途数；并发默认 `1` 以保持旧行为。thinking/reasoning 参数仅在显式配置时深度合并到 proofread 请求，不影响 translate/glossary。兼容 provider 可用较强推理提高疑难问题覆盖，但代价是 token、延迟和费用；留空使用 provider 默认。报告中的最终决策计数用于可观测性，不得被当作目标修改率。
...
`glossary.md` 是完整常驻的全局硬规则；retrieved_context 只能补充，不能截断或替代它。`<base>.web_evidence.json` 的 `confirmed_terms` 是经网页正文映射、来源质量和冲突规则验证后的持久化 source→target 映射，并分两层权威：glossary 阶段证据可参与构建全局术语；proofread 阶段证据只保留在项目 sidecar，作为局部可追溯 safety 约束，不得回写 glossary 缓存、fingerprint 或全局知识。glossary 的全局规则优先，命中的 confirmed term 次之，retrieved_context 优先级最低；任何冲突都必须 human review，不能静默漂移或改写 glossary。`confirmed_terms` 随 sidecar 跨重跑保存，并在 proofread 的初次候选和 safety retry 前从最新 sidecar enrich。启用 `EMBEDDING_ENABLED=1` 时，Chroma 索引同时包含 `glossary:*` 项目知识 chunk、`web_evidence:*` 网页证据 chunk、`transcript:*` 源文 chunk 和翻译/分割后生成的双语 `translation_memory:*` chunk；这些按当前字幕逐条召回为 `retrieved_context`。proofread 阶段用源文+译文 query 检索，优先获得历史译法和术语一致性参考。
